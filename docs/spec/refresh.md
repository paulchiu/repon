...
The thread limit above was measured; the pool width, rayon's global pool sized at whatever `available_parallelism()` returns, never was, which is the gap [#361](https://github.com/paulchiu/repon/issues/361) closed with a reproducible harness rather than another hand-run number: `tools/fanout-sweep`, a corpus generator and sweep driver kept in its own workspace outside `just ci` and run by hand with `just sweep-fanout` (synthetic) or `just sweep-fanout-real` (read-only, against real repositories the caller names). Swept over pool width {1, 2, 4, 8, 12, 18, 36} against gix's own thread limit {1, 2, 4, none}, on synthetic corpora built to this document's own shape (one outlier near `vial-qmk`'s file count, a heavy tail, a mass of small clean repos, a small dirty minority) and checked read-only against the owner's real 202-repository population, the width turned out to be a broad plateau from about 4 workers up through 36 (an 11-13% band at a corpus large enough to approach the real population's own size) rather than a sharp optimum, and leaving gix's own limit unbounded stayed the worst or tied-worst choice at every width from 2 upward, reconfirming the pin above. Composed with a simulated concurrent fetch or Action, no single width won in every regime, but a pool of 18, the physical core count and exactly what the global pool's free default already gives, was never more than about 10% off the best cell in any of them, where a hand-picked narrower or wider constant each lost by more in at least one. [ADR 0013](../adr/0013-no-filesystem-watching-a-refresh-is-a-cancellable-generation.md) carries the full measurement and the machine it ran on; the conclusion is that the width stays exactly what it already was, now for a checked reason rather than an unmeasured default.
