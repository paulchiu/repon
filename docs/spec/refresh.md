...
While a cycle runs it reports how many of the repositories it fans out over have finished, at [layout-and-provenance.md](layout-and-provenance.md#the-status-row)'s rank 3: `fetching 3/12`, where the denominator is the cycle's own population rather than the table's entity count, and the numerator counts finished attempts, a failure being as finished as a success. A fetch is network bound and spends seconds at a time on one remote, so the count has something to say here that it would not on a Refresh. It is read off the running cycle every frame rather than landed by a generation, so it moves while the cycle does; it is gone once the cycle is taken back, leaving the bare verb.

The same cycle runs on demand, on any chord dispatching `Action::FetchNow` (`f` by default), rather than only when this section's own timer says so. It is this cycle unchanged: it always prunes, fails closed on credentials, touches nothing in the working tree, is bounded to `fetch.concurrency`, counts its failures the same way, and starts the same normal generation on completion, and the auto-update rides it exactly as it rides a tick's. Two rules are its own. It is ungated by `fetch.enabled`, which governs the cycle Repon runs unbidden on a timer and never one the user asked for, the same line [config.md](config.md)'s built-in `sync` already sits on; `--no-fetch` needs no separate rule, since [config.md](config.md)'s "The command line" defines it as forcing `fetch.enabled` off and gives it no meaning beyond that. And a press landing while a cycle is in flight is refused rather than queued, the same choice a tick arriving mid-cycle already makes, which is not the single immediate cycle enabling the fetch owes: that one is an obligation held until it can be met, where a repeated gesture against a fetch already running has nothing left to ask for. The status row reports it at [layout-and-provenance.md](layout-and-provenance.md#the-status-row)'s rank 3, counting as it goes, the same way it reports a tick's own cycle: the two are the same cycle and neither is special cased there.
