...
| The fetch key | A fetch-and-prune cycle now rather than at the next `fetch.interval` tick, on any chord dispatching `Action::FetchNow` (`f` by default). Not a generation of its own: it is the same cycle "The periodic fetch" below specifies, so the generation arrives on that cycle's completion by the row already in this table. Refused rather than queued while a cycle is in flight. |
...
Only the two refresh keys carry the hook, and the rest of this table deliberately does not. Terminal focus gained is best effort and terminal dependent, so a script would run or not run depending on a multiplexer option; a resume is the moment the user was doing inner-loop work by hand; and a periodic fetch's own completion is a background tick nobody aimed at anything. All three are Repon deciding rather than the user asking, which is the line [0002](../adr/0002-repon-owns-the-outer-loop-only.md) draws. The fetch key is the one aimed gesture in this table that still does not carry the hook, and for a different reason: the hook is `on_refresh`, wired to the two Actions that dispatch a Refresh, and a fetch is a different gesture with a name of its own. Firing a hook called `on_refresh` from it would make the field's name a lie about when it runs. "The refresh key" names an Action, not a character: a chord dispatching `Action::RefreshAll` or `Action::RefreshSelection` carries the hook whether it is `r`, `F5` or a user's own rebind, because the hook is wired to the Action the merged binding table dispatches rather than to the keystroke itself.
...
Unknown carries a reason, and the detail pane reports it in words: `timed out`, `no default branch found`, or `not yet initialised` on an uninitialised Submodule row ([discovery.md](discovery.md)). All three render `?` in the gutter. This sentence previously listed four reasons; [0019](../adr/0019-a-detached-head-is-a-shape-of-head-not-a-worktree-state.md) removed `NoUpstream` and `NoRemote`, and `SubmoduleUninitialized` arrived after it, so [core-api.md](core-api.md) records the set as closed at three. `unknown:timed-out` reaches the first from the Filter line ([filter.md](filter.md)); the third has no keyword there yet.
...
A cycle carries a count of how far it has got, whichever started it: how many of the repositories it fans out over have finished, against the population it started against. The denominator is the cycle's own rather than the table's entity count, since an excluded Repo is listed and never fetched, and Repos sharing one common dir are fetched once between them. The numerator counts finished attempts, a failure being as finished as a success, and a cancelled cycle's skips count as neither, having never been attempted. A consumer reads it off the running cycle rather than being handed it by a generation, so it moves while the cycle does and is gone once the cycle is taken back.

The same cycle runs on demand, on any chord dispatching `Action::FetchNow` (`f` by default), rather than only when this section's own timer says so. It is this cycle unchanged: it always prunes, fails closed on credentials, touches nothing in the working tree, is bounded to `fetch.concurrency`, counts its failures the same way, and starts the same normal generation on completion, and the auto-update rides it exactly as it rides a tick's. Two rules are its own. It is ungated by `fetch.enabled`, which governs the cycle Repon runs unbidden on a timer and never one the user asked for, the same line [config.md](config.md)'s built-in `sync` already sits on; `--no-fetch` needs no separate rule, since [config.md](config.md)'s "The command line" defines it as forcing `fetch.enabled` off and gives it no meaning beyond that. And a press landing while a cycle is in flight is refused rather than queued, the same choice a tick arriving mid-cycle already makes, which is not the single immediate cycle enabling the fetch owes: that one is an obligation held until it can be met, where a repeated gesture against a fetch already running has nothing left to ask for. The status row reports it at [layout-and-provenance.md](layout-and-provenance.md#the-status-row)'s rank 3, counting as it goes. Rank 3 is a gesture's own slot and appears only once a key has fired, so a tick's cycle carries the same count and simply has nobody asking after it there.
