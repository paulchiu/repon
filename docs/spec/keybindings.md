...
That absence is not guessable from the screen alone, so the field's own bottom border carries a reminder while `Stage::Choosing` is what it draws: `no shell: $VAR and $(cmd) are literal; use sh -c '...'`. Shell syntax typed into the field is inert, an ordinary argv element `shell-words` unquotes rather than expands, but the environment is real regardless: all eight `REPON_` variables are on the child's own environment either way, so a program that reads `REPON_REPO_PATH` itself still sees it, and it is only writing `$REPON_REPO_PATH` into the typed command that stays a literal four characters. `sh -c '...'` is the escape hatch already, needing no new key: argv-split to three elements, it runs exactly what was meant with the shell visible in the command itself, which is the audit trail [0007](../adr/0007-launchers-are-argv-vectors.md) asks for. A frame too narrow for the whole sentence drops from the least essential clause first, the same discoverability-cost reasoning [the footer](#the-footer) applies to its own hints: the `$VAR`/`$(cmd)` explanation goes first, then `use sh -c '...'`, and `no shell` itself is the one clause that never drops once anything is drawn at all.
...
At a frame wide enough, the same content lays out in two columns of key/description pairs side by side instead of one long scrolling list: a reading surface's expensive part is scrolling, and a wide terminal has the width to spare for a second column instead. Wide enough is derived from the content itself, not a round number picked by hand: twice the widest line the whole unfiltered content actually renders (the same line, measured the same way the key/glyph column's own fixed width above is), plus one gutter between the two columns. The key/glyph column's own width is the one figure computed above, shared across both columns rather than measured separately per column, which is what keeps the two columns' own gutters lined up with each other as well as within themselves.
