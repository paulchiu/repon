...
So the ad hoc field now defaults to `$SHELL -c`: each non-empty line is one step, and (unless the field's own toggle has turned shell mode off) the whole line is handed to the user's shell rather than split with shell-words first, per the `shell = true` convention [config.md](config.md) already fixes for a config step (`argv` holding exactly one element, the whole command string). The lines still gate exactly as config steps do, and a newline typed with `Alt+Enter` ([keybindings.md](keybindings.md#the-ad-hoc-command-field)) means exactly what a pasted one or one returned by `$EDITOR` means. `Alt+S` toggles the mode off for the run about to happen, resetting to on every time the palette reopens, since a sticky off-state nobody remembers reproduces the exact silent no-op the default exists to remove.
...
The same `shell` field is on `StepResult` for a config step too, which also has two modes and also showed only its command string before this. One field and one rendering path serve both origins, since the detail pane has no way to tell an ad hoc step from a config one apart from this.

One surprise worth documenting rather than leaving to be discovered: `$SHELL -c` runs the user's own login shell non-interactively, and for zsh that sources `.zshenv` and not `.zshrc`, so an alias or function defined only in the latter is absent. Shell mode therefore behaves almost, but not always exactly, like the terminal the command was drafted in.
