...
Three rules govern a redrawn line, because under a PTY ONLCR means every newline arrives as `\r\n`, and a spinner can ask for the redraw either of two ways. A `\r` immediately before a `\n` is a line ending and its CR is dropped. A bare `\r` is a progress-frame separator, and only the last frame of the sequence is kept, which is what turns an animated progress bar into its final state rather than a concatenation of every repaint. A CSI erase-in-line (`ESC [ K`, with or without a `0`, `1` or `2` parameter) or cursor-to-column-1 (`ESC [ G`, `ESC [ 1 G`) sequence is the same progress-frame separator spelled the other way, and is collapsed identically; `npm install`'s own spinner redraws this way, not with a bare `\r`. Cursor-up (`ESC [ A`) is left alone: placing it correctly needs a real screen model, which capture does not keep, so it reaches the pane unresolved rather than guessed at. Every other CSI sequence, SGR (`ESC [ ... m`) included, is never a separator and is never rewritten: it reaches the pane exactly as the step wrote it, unless it was written into a frame a later separator discards, in which case it goes with everything else that frame wrote.
...
Transient capture is larger than the receipt it produces, because the whole raw stream is held while the child runs, normalisation allocates a second copy of it, and only then are the head and the tail cut. Measured standalone, `rustc -O` over copies of the normalise and bound functions, peak resident set above an idle process, at a fixed 119-character line (121 bytes on the wire once ONLCR has been applied): 121 KB emitted peaks 0.5 MB and spends 0.2ms normalising and bounding, 12.1 MB peaks 26 MB and 15ms, and 121 MB peaks 259 MB and 153ms, so the peak runs at about twice what the child emitted. Both costs follow the bytes emitted rather than the line count: the same 121 MB in 1,000 lines rather than 1,000,000 costs the same 0.15s, and peaks higher only because the bound then keeps 400 lines of 119,000 characters. Against the real outputs above, the largest of them 25,598 bytes, that is under 60 KB of transient and under a tenth of a millisecond, which is why the stream is held whole rather than normalised as it arrives.

That ruling is about how much a step writes, not how long it runs. A repainting step that never exits holds every byte it ever wrote, and there is no per-step timeout to end it: `top` under a PTY emits 9 KiB/s, some 32 MB an hour, which normalisation then doubles at the end. Normalising as the bytes arrive would not bound that case either, because only cutting them as they arrive would, and the bound cannot cut until a line ends.
...
A config step's `shell = true` is visible in the entry that carries it, which is the audit trail [0007](../adr/0007-launchers-are-argv-vectors.md) requires: a shell is opted into by an explicit per-entry flag, making the risk visible in the file rather than the default, and `shell = false` stays the default there ([config.md](config.md)'s "Actions"). This does not extend to the ad hoc field: it has no config entry to carry a flag in, and [#351](https://github.com/paulchiu/repon/issues/351) narrows 0007's rule to config steps and Launchers rather than repealing it, because a typed command that reads like a prompt behaving like an argv builder is a silent wrong answer, not a safe default. `echo $(pwd)` typed and run used to exit zero having printed the four literal characters `$(pwd)`, the quietest failure mode there is.

So the ad hoc field now defaults to `$SHELL -c`: each non-empty line is one step, and (unless the field's own toggle has moved off the default) the whole line is handed to the user's shell rather than split with shell-words first, per the `shell = true` convention [config.md](config.md) already fixes for a config step (`argv` holding exactly one element, the whole command string). The lines still gate exactly as config steps do, and a newline typed with `Alt+Enter` ([keybindings.md](keybindings.md#the-ad-hoc-command-field)) means exactly what a pasted one or one returned by `$EDITOR` means. `Alt+S` cycles the toggle through three states rather than two, `Shell -> Off -> Interactive -> Shell`, resetting to `Shell` every time the palette reopens, since a sticky `Off` or `Interactive` nobody remembers reproduces the exact silent no-op the default exists to remove, or pays `Interactive`'s own rc-sourcing cost by accident ([#414](https://github.com/paulchiu/repon/issues/414)). `Interactive` is `Shell` again but through `$SHELL -ic`, the same key a config step's `interactive` field runs under ([config.md](config.md)'s "Actions").

The regression surface this accepts is real and stated rather than hidden. A command with no shell metacharacters behaves identically either way: shell-words unquotes and a real shell unquotes, and they agree on the ordinary cases including a quoted argument with a space. The two modes only diverge where a `$`, a backtick, a glob, a pipe or a redirection appears, which is precisely the set of cases the old default got wrong. The sharper cost is an unset variable: it no longer errors, it expands to nothing, so `git reset --hard $BASE` with `BASE` unset goes from `fatal: ambiguous argument` on every row to `git reset --hard` on every row, silently, across the whole fan-out. The confirm gate cannot show that expansion, because it differs per Repo; what it can do, and now does, is name the mode plainly beside the command and the count it is about to run against. The toggle is the escape hatch, and the mode reaching three places, the field's own chrome, the confirm gate's text and the receipt (`StepResult::shell` below), is what pays for the trade rather than leaving it invisible the way the old rule's one gap did.

The same `shell` and `interactive` fields are on `StepResult` for a config step too, which also has these modes and also showed only its command string before this. One pair of fields and one rendering path serve both origins, since the detail pane has no way to tell an ad hoc step from a config one apart from this.

One surprise worth documenting rather than leaving to be discovered: `$SHELL -c` runs the user's own login shell non-interactively, and for zsh that sources `.zshenv` and not `.zshrc`, so an alias or function defined only in the latter is absent. `interactive = true` (a config step's own key, or the ad hoc field's `Interactive` toggle state) is the answer to exactly that gap, running `$SHELL -ic` instead so the rc file is sourced first; it stays opt-in rather than folded into `shell = true` because that rc file is arbitrary user code, paid once per step per operable Repo, and can be slow (`compinit`, a version-manager hook) or noisy (writing to stderr, which a captured PTY shows as step output) in a way a non-interactive shell never is.
...
pub struct StepResult {
    ...
    pub shell: bool,               // whether `label` ran through `$SHELL -c`; false for own work
    pub interactive: bool,         // whether that shell was `-ic`; meaningless, and false, unless `shell`
}
...
```
 91  work 242 entities · run 7/12 · filter: 12 matches · worktrees: 161 (preference off) · 12.0s
 87  work 242 entities · run 7/12 · filter: 12 matches · worktrees: 161 (preference off) ...
 53  work 242 entities · run 7/12 · filter: 12 matches ...
 32  work 242 entities · run 7/12 ...
 21  work 242 entities ...
 17  work 242 entities
```

The detail pane carries the run itself: the labelled per-step output of the last Action, each step separately readable, surviving the run, as [layout-and-provenance.md](layout-and-provenance.md) already promises, plus per-step elapsed time, which shows a stuck step in the absence of a timeout. A running step carries the spinner in the same position the step number's outcome will occupy. Beside the list, the pane is 106 columns in a 140-column frame, 104 interior:
...
`q` is gated behind a confirm while a fan-out is in flight, because quitting orphans the children ([keybindings.md](keybindings.md)). It is the only quit there is: Repon binds no chord of its own to suspending, so nothing stops a fan-out reversibly and the confirm is the whole of the protection.

This is also the genuine conflict in the settled specs, named as such. [The core API spec](core-api.md) assigns Action fan-out to the core, [refresh.md](refresh.md) stops all background work while the terminal is handed to a child, and `pause` and `resume` exist with the core contractually not told why. For probes that ignorance is free, because cancellation is 0.5 to 0.9ms and the work is idempotent and re-runnable. For a child process it is not: holding means SIGSTOP then SIGCONT, quitting means SIGTERM then SIGKILL, and `pause` is forbidden the information that tells them apart. The resolution: `pause` stays ignorant, and the fan-out gets its own hold and stop verbs on the core, so nothing needs a reason and the core API's rule stands. The hold verb has no caller in the terminal interface today, since the chord that drove it is gone; it stays on the core as the answer to the question `pause` is forbidden, which is a property of the core API rather than of any binding.

One Action runs at a time, and five keys over four surfaces go inert while it does. There is no `running` context among [keybindings.md](keybindings.md)'s six, so nothing previously forbade `;` or `m` opening the Action palette again, `s` or `1` to `9` switching Set and re-running discovery so in-flight step results arrive for EntityKeys no longer in the table with no Generation to arbitrate them, or Ctrl+R reloading config, which re-applies `[[action]]` immediately and can delete the running one. So `;`, `m`, `s`, `1` to `9` and Ctrl+R are inert while a fan-out is in flight. `m` joined that list when [repo-management.md](repo-management.md) landed, and for the identical reason `;` is on it: `m` opens the same palette, filtered to the built-in management operations, so it is one surface with two ways in and answers with the same Notice. `!` stays live, because handing one Repo to lazygit while another installs is a thing a person may legitimately want. The honest cost: this needs a binding conditional on runtime state, and the compiled table is keyed on context alone. The footer is derived and follows along fine, but the help overlay and the load-time collision check are over the static table, and 0016 spent a paragraph rejecting lazygit's disabled-reason mechanism as the thing that makes the escape hatch vanish where a user is most lost. [0018](../adr/0018-an-action-is-a-fanout-of-pty-backed-steps.md) prices this openly.
