...
Per-Repo applicability narrows that count further, and it is decided though not yet specified as a schema ([#172](https://github.com/paulchiu/repon/issues/172)). An Action carries a predicate in the Filter grammar, evaluated over already-settled Cells, so the palette reads `run "reinstall" on 8 of 12 selected` rather than the Selection count alone. Nothing new is parsed and nothing new is read: [0022](../adr/0022-the-filter-language-is-total-and-three-valued.md)'s language is total, so every string is a valid predicate and there is no fifth failure grade, and it is three-valued, so a Repo whose Cells have not settled is unprovable rather than inapplicable and is reported as an unresolved tail instead of being silently dropped. That last part is what keeps the count honest while a Generation is still in flight, which is [0001](../adr/0001-per-cell-provenance.md) reaching the palette.
