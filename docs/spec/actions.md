...
Run progress lives in the header, and the header gains the degradation rule it never had. [0006](../adr/0006-no-git-state-cache-session-state-by-name.md) requires a match count, [the config spec](config.md) requires `worktrees: 161 (preference off)` beside it and a restored Filter announcing its own count, and composed that is already 85 columns against an 88-column narrow screen and a 90-column list; run progress makes it 107. The footer got four explicit rules and a measured drop table in [keybindings.md](keybindings.md), and the header got nothing, so this spec closes the gap with the same mechanics: items separated by ` · `, the ASCII ` ...` ellipsis reserved inside the budget rather than appended after it, every item width-checked including the first, and the last surviving item dropping the ellipsis rather than itself. Priority while a run is in flight: the entity count, then run progress, then the Filter's match count, then the worktrees note, then timing.

```
 94  repon 403 entities · run 7/12 · filter: 12 matches · worktrees: 161 (preference off) · 12000ms
 88  repon 403 entities · run 7/12 · filter: 12 matches · worktrees: 161 (preference off) ...
 54  repon 403 entities · run 7/12 · filter: 12 matches ...
 33  repon 403 entities · run 7/12 ...
 22  repon 403 entities ...
 18  repon 403 entities
```
...
- The ascii glyph set defect, named so it is not rediscovered. [theming.md](theming.md) claims one switch and two vetted sets, but the ascii set is listed nowhere in `docs/`, and the loading spinner is braille, so under `glyphs = "ascii"` an in-flight Action, and every refresh, has no progress mark at all. That is the [0010](../adr/0010-provenance-renders-as-a-row-gutter-and-blank-cells.md) disjointness class of defect sitting in the spec today. Enumerating a second vetted set is theming's question, so it is deferred to its own ticket rather than settled here.
