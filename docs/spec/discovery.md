...
A repository deliberately nested inside another repository's working tree is not reached by the walk, because the walk stopped at the outer boundary. It is reached by adding its parent directory as a Set `root`. That is the only escape hatch, and it is deliberate: the boundary rule stays absolute, and the config names the exception rather than the walk guessing at one. Reaching an agent's own checkouts below works that case through for the tooling that most often creates one.
...
## Reaching an agent's own checkouts

Where agent tooling creates its checkouts inside the repository it is working in, they sit under a directory of its own, `.claude/worktrees/` and the like. Every one of them is the vendored case in "What is never a row" above: the walk stops at the project's own `.git` and never sees what sits below it, which in the measured population accounts for 19 of the 38 entities boundary-stopping misses.

Naming the containing directory as a Set `root` reaches them, the escape hatch the walk above already allows. A root is walked from the directory itself, and a root that is a boundary is recorded and descended no further, so a root naming the containing directory finds every checkout one level down. Roots are deliberately not deduplicated against one another, so a root inside another root's territory adds what the outer one stopped short of rather than colliding with it:

```toml
[[set]]
name = "agents"
roots = ["~/dev", "~/dev/manage/.claude/worktrees"]
```

What lands in the table depends on what created the checkout, and gix's own repository kind decides that when a boundary is resolved, never the walk, which reads only whether a `.git` entry is there at all. A checkout `git worktree add` created is a linked Worktree: it shares its parent's git common dir, so it groups and indents under that parent wherever on disk it sits, and `sync` refuses it ([repo-management.md](repo-management.md)'s "What `sync` refuses, and why"). A checkout `git clone` created is a Repo row of its own that `sync` acts on like any other, and so is a submodule-shaped checkout, whose `.git` file resolves to a Repo the same way a Submodule's own does. All 19 in the measured population are clones, of other projects rather than of the one containing them.

Keep the parent repository in the same Set as its Worktrees. A Set whose roots reach the checkouts but not the repository they belong to still lists them, appended after every group as children with no parent rather than dropped ([layout-and-provenance.md](layout-and-provenance.md)'s "The list").

A Worktree created beside its parent needs none of this. It sits above every boundary, so the ordinary walk finds it with nothing declared, which is how all 163 linked Worktrees above are found. A checkout inside a working tree costs one `roots` entry per repository that holds one, since `roots` are literal paths with `~` expansion and no globs ([config.md](config.md)'s Sets).
