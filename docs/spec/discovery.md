...
Agent tooling puts the checkouts it creates under a directory of its own inside the working tree, `.claude/worktrees/` and the like. Every one of those is the vendored case above: the walk stops at the project's own `.git` and never sees what sits below it. In the measured population they are the largest family it misses, 19 checkouts under `manage/.claude/worktrees/` against 17 Submodules and two one-offs.

Naming the containing directory as a Set `root` reaches them, the one escape hatch "The walk" above already fixes. A root is walked from the directory itself, and a root that is itself a boundary is recorded and descended no further, so a root naming the containing directory finds every checkout one level down. Roots are deliberately not deduplicated against one another, so a root inside another root's territory adds what the outer one stopped short of rather than colliding with it:
...
What lands in the table depends on what the tool created, and the `.git` entry says which, the same file-or-directory distinction "The walk" above already turns on. A `.git` file is a linked Worktree, sharing its parent's git common dir, so it groups and indents under that parent wherever on disk it sits, and `sync` refuses it ([repo-management.md](repo-management.md)'s "What `sync` refuses, and why"). A `.git` directory is an independent clone, a Repo row of its own that `sync` acts on like any other. All 19 in the measured population are the second shape, and they are clones of other projects rather than of the one containing them.

The two layouts cost differently, and the difference is config rather than capability. A Worktree created beside its parent sits above every boundary, so the ordinary walk finds it with nothing declared, which is how all 163 linked Worktrees above are found. A checkout inside a working tree costs one `roots` entry per repository that holds one, since `roots` are literal paths with `~` expansion and no globs ([config.md](config.md)'s Sets), so ten such repositories are ten entries and every new one is another.
