...
- Five rows are chrome at every frame height: the status row, the frame's top border, the column header, the frame's bottom border and the footer. At the 24-row reference terminal that leaves 19 rows for the table at `88x24`, and nothing else takes a permanent row. A tab strip and a dedicated warning row were each refused on that arithmetic, since both would spend one of the 19 for the whole session on state that changes twice in it; the Set picker and the reserved `[N]` indicator are what stand in their place.
...
Rank 3 answers "did my keypress land", for the refresh key and the fetch key alike (`r`, `F5`, `R` and `f`, or a user's own rebind of any of them). A Refresh reads `refreshing all 403` or `refreshing selection 5` while [`Core::refresh_running`](core-api.md) still reads true for the Refresh that key dispatched, and `refreshed all 403` once it settles. A fetch reads `fetching 3/12` while [`Core::fetch_progress`](core-api.md) holds a count and `fetched`, bare, once the clock takes the cycle back. The count is the cycle's own, not the table's: `total` is the population it fans out over and `done` is the attempts that have finished, failures included. It goes away on settling rather than resting at `12/12`, the same "present only while in flight" rule run progress follows, since a finished cycle's count says nothing a verb does not. The row holds one of the two, the most recent, since it reports what the user last asked for rather than everything that has run: a fetch replaces a Refresh's report here exactly as a second Refresh already replaces the first. It ranks below the warning message for the same reason the message outranks everything under it, and above run progress because a gesture in flight is the more immediate fact; a Refresh and a run rarely overlap in practice, since starting an Action cancels any Refresh already running ([refresh.md](refresh.md)'s "Starting an Action"). A Refresh carries no fraction and a fetch does, which is a distinction between the two gestures rather than an inconsistency: phases A and B cover the whole population in about 0.15 seconds ([refresh.md](refresh.md)'s "The phases"), so a Refresh's live count would jump from nothing landed to everything landed with no readable state between, the defect refresh.md already recorded once for a static per-row spinner, where a fetch is network bound and spends seconds at a time somewhere a count can actually name. It persists once settled, unlike run progress, which is the point: a Refresh over an already-populated table commonly finishes inside the frame that started it, and the settled text is what a user pressing `r` on such a table actually gets to read. It appears from the moment either key first fires this session and is replaced, never cleared, by the next dispatch.

Vanished entities are the mirror of an abandoned discovery and stand as a warning for the same reason ([#171](https://github.com/paulchiu/repon/issues/171)): rows are present that no longer exist, and their values are frozen. This also makes a Vanished row discoverable at all, which the gutter structurally cannot do, since a mark on a row does not tell a user the row is there. The condition announces itself here, `presence:vanished` is the way in, and `d` is the way out.
...
Each column opens at its own natural direction the first time it is chosen, and the same key again reverses it. One rule fixes all six: **`name` and `branch` open ascending, A to Z, and the four columns that count trouble, `sync`, `base`, `dirty` and `state`, open descending, because the reason to sort by a count of trouble is to bring the worst rows to the top.** A column chosen while another one is active opens at its own natural direction and never inherits the previous column's.

Four of the six rank by something already on screen. `name` and `branch` compare the cell's own text lowercased, so `Zed` and `apex` fall where a reader expects them rather than where their code points would put them. `base` compares the commits-behind count the cell shows, and `dirty` the changed-path total, read from the same three typed counts the cell renders, so the order and the cell can never disagree about what dirtier means.

The other two have no single number to rank by, so their orders are written out here rather than taken from an enum's own declaration order, which is a narrative order and would make a ranking by accident. **`state` ranks worst last, so the natural descending direction opens on the worst: Gone, whose upstream is deleted, so nothing on the remote remembers the branch; then Merged, landed and safe to sweep; then Local only, never pushed and so never safe to sweep; then Active, unlanded pushed work, the ordinary state with nothing to do about it.** **`sync` ranks as a triple, a rank then the behind count then the ahead count**, with no remote lowest and no upstream next, so descending opens on the rows furthest behind their upstream and sinks the rows with nothing to compare against to the bottom.
...
A child row is indented under its parent and marked `└`, or `` ` `` under `glyphs = "ascii"` ([theming.md](theming.md)), and a Submodule row carries the same mark as a Worktree row rather than one of its own. A child name gets the name column's width less the four columns the indent, marker and gap spend: 28 minus 4 = 24 columns at the column's minimum, 23 behind a two-character marker, and 40 minus 4 = 36 once the column has grown to its cap. The screens below previously drew Submodules with `∙` (U+2219), which sits one codepoint from `·` (U+00B7), the clean value, on the same row; that is the disjointness rule failing in the value plane rather than through provenance. The cost is that a Submodule row and a Worktree row look alike, and the name column and the detail pane are what tell them apart. Submodule rows are settled in [discovery.md](discovery.md).

That connector points at the row directly above it among the rows actually drawn on screen, after filtering, sorting and scrolling alike, not at whatever the snapshot or the unwindowed candidate order happens to hold: it is drawn only when the row above is this child's own Repo, or another of that Repo's own children already carrying the connector, so an unbroken run of a group's rows reads as connected all the way back to a Repo that is actually on screen. A child with no such row above it, whether it sits first with nothing above it on screen at all, a Filter or sort has put an unrelated row there instead, or scrolling has carried its own Repo off the top of the viewport, draws a second marker in the same column, one display column wide like the connector it replaces: `┆` under `full`, `:` under `ascii` ([theming.md](theming.md)). The child is still marked as a child either way, since the indent and the marker are what tell `manage-cad-1958` the Worktree from a Repo of that name, but the marker no longer claims a parent the row above does not show. `kind:worktree` is the case that never shows the connector at all: no Repo row is ever a candidate under it, so every surviving row draws the orphan marker.
