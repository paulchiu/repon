...
Columns are left-packed rather than right-aligned to the frame edge: a one-character Selection marker, name 28, branch 24, sync 9, base 6, dirty 6, state 10, then a filler column absorbing the slack. With the gutter and single-space gaps that is 92 columns before the filler. The one-character gutter precedes the marker and carries the row's least-settled provenance state.
