...
Rank 3 answers "did my keypress land", for the refresh key and the fetch key alike (`r`, `F5`, `R` and `f`, or a user's own rebind of any of them). A Refresh reads `refreshing all 403` or `refreshing selection 5` while [`Core::refresh_running`](core-api.md) still reads true for the Refresh that key dispatched, and `refreshed all 403` once it settles. A fetch reads `fetching` while [`Core::fetch_running`](core-api.md) holds and `fetched` once the clock takes the cycle back, with no count beside it, because a cycle decides for itself which Repos have a remote to reach. The row holds one of the two, the most recent, since it reports what the user last asked for rather than everything that has run: a fetch replaces a Refresh's report here exactly as a second Refresh already replaces the first. It ranks below the warning message for the same reason the message outranks everything under it, and above run progress because a gesture in flight is the more immediate fact; a Refresh and a run rarely overlap in practice, since starting an Action cancels any Refresh already running ([refresh.md](refresh.md)'s "Starting an Action"). It carries no fraction of entities settled: phases A and B cover the whole population in about 0.15 seconds ([refresh.md](refresh.md)'s "The phases"), so a live count would jump from nothing landed to everything landed with no readable state between, the defect refresh.md already recorded once for a static per-row spinner. It persists once settled, unlike run progress, which is the point: a Refresh over an already-populated table commonly finishes inside the frame that started it, and the settled text is what a user pressing `r` on such a table actually gets to read. It appears from the moment either key first fires this session and is replaced, never cleared, by the next dispatch.
