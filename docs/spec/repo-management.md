...
A run outstanding refuses every key that would start a second one, exactly the same set an Action's own fan-out already refuses ([keybindings.md](keybindings.md)'s "Quitting and confirming"): `;`, `m`, `s`, `1` to `9`, `Ctrl+R` and `e` answer with a Notice rather than silence, and `q` asks for confirmation first, since quitting mid-run abandons the background thread. Every other key and movement is read exactly as it would be with no run outstanding at all.

The Filter is the one exception, and only for the rows the run itself still has ahead of it: a row currently executing, or still to come in the Selection's own order, stays visible even once its own Cells stop matching the Committed Filter, so a `sync` fast-forwarding a row cannot make it vanish mid-hook merely because the unrelated periodic poll ([refresh.md](refresh.md)'s "The poll") happened to re-probe it first. The row's own Cells still update and repaint live; only its membership in the filtered list is held. It drops the instant the run's own progress marker moves past it, on that very frame, never lagging to the next refresh or keystroke, and once the run finishes every row it touched is judged by the Filter alone again. This pin never widens `show_worktrees` or `show_submodules`, is never counted toward the header's own match count, and is never swept into the Selection by `a`: it holds a row past the Filter, it never claims the row satisfies it. See [filter.md](filter.md)'s "What a Filter does to the list" for the identity this is the named exception to.
