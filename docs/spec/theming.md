...
Every one of the nine is a foreground colour. Backgrounds are absent everywhere except the selected row, and the two keys above are the whole of that exception: setting a background is how a theme stops inheriting the terminal's own and starts fighting it, which unwinds the correction layer one key at a time. There are no paired light and dark variants and no `appearance` key either, because the default names the user's own sixteen slots and their own foreground, so it reads correctly on both grounds without Repon detecting either.
...
This does more than set an accessibility floor for the red and green pair. crossterm honours `NO_COLOR` automatically inside `SetForegroundColor`, so `NO_COLOR=1 repon` drops every colour with no code of ours involved. The rule keeps that screen monochrome and complete rather than monochrome and lying.
...
The warning slot carries **standing conditions of the session only**: a theme that half-applied, a config key that fell back, an `on_refresh` Action whose last run failed a step, a periodic fetch cycle that failed on some repositories, an abandoned discovery, entities that vanished and are still listed. Each is continuously true until something changes it, and each puts something already on screen in doubt. That makes ranking them against each other meaningful, and expanding them into a list on `w` coherent. [0023](../adr/0023-an-unbuilt-binding-is-not-advertised-and-an-unavailable-one-answers-on-press.md) fixes the rule after a fourth source, a bound-but-unbuilt key the user had just pressed, was ranked into the slot and was then invisible at the moment it fired.
...
Where a Notice and a warning sit on the status row, and what happens when they and the header do not all fit, belongs to [layout-and-provenance.md](layout-and-provenance.md#the-status-row) and lives there in full. It is not restated here, because a rule kept in two places is how this document came to fix the order in one sentence and disclaim it four lines later ([0026](../adr/0026-the-status-row-is-one-list-not-a-stack-of-surfaces.md)). Two of its rules reach back into this one. A Notice takes the whole row alone, which is why its text is authored to a budget below rather than truncated by the renderer. And a warning survives every width as a reserved `[N]` indicator, which lets the slot's own message be dropped at a narrow width without the condition being hidden: the message is a convenience, the indicator is the honesty.
...
- `` ` `` is GNU tree's, whose ascii table is `` `-- `` for a last child and `|-- ` for a middle one, with one-character forms `` ` ``, `+` and `|`. The marker must be one character: a second character costs the child budget one more column at every name-column width, so it always truncates more of the 183 child rows than the one it would replace (68 of the 183 child rows in the measured population, at the column's minimum; [layout-and-provenance.md](layout-and-provenance.md#the-list) carries the budget arithmetic). The cost: `` ` `` hangs at the top of the cell where `└` hangs at the bottom left, and it is a legal branch-name character.
...
The Action palette therefore draws its border in `warn` and puts the Selection count in the border title, so it reads `run on 12 selected` before anything is typed. That is the reading with nothing narrowing it. Once an entry declaring a `when` is highlighted or chosen, the same title narrows to the applicable count and reports its unresolved tail instead; [actions.md](actions.md)'s "The Selection and the gate" carries the exact reading of each, since the counting is that document's and only the colour is this one's. The Launcher palette keeps `border_focused` and names the one Repo. No tenth role and no second border style for a narrowed count: the count is the signal and the colour is emphasis, which keeps it working under `NO_COLOR`.
