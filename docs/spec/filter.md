...
| `action:ok\|failed\|cancelled\|none` | the worst `StepOutcome` in `last_action`, or its absence |
...
- **`action:` is the receipt, not the fold.** `action:failed` selects the identical set that `n` and `N` walk ([actions.md](actions.md)), because both read a `Failed` step in `last_action`. `action:none` is a row no Action has touched this session.
...
- **`unknown:` is the only reason key**, because `enum Unknown` is the only closed reason set in the design (core-api.md:80, two variants, versioned by the wire `schema` integer). `ProbeError`'s variants are unspecified, and a `reason:` key matching rendered error text was refused: it would make the language depend on error wording, and it would decide a type [core-api.md](core-api.md) owns. When those variants are specified, `failed:<variant>` slots into this shape with no change to the grammar.
...
- **`is:selected`.** It puts the Filter and the Selection in a loop with `a`, and CONTEXT.md's "a Filter never mutates the Selection" is worth keeping obviously true rather than subtly true.
