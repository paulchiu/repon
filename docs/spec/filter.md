...
`Alt+/` also fires from inside the line itself, bound there separately since `global` is suspended in `input` and could never carry `list`'s own row across ([keybindings.md](keybindings.md)'s "The contexts"). It closes the line the same way Esc does, but clears the committed Filter it closes over instead of restoring it, so the line always reopens over nothing rather than whatever was last applied. Esc and `Alt+/` stay two distinct gestures for exactly this reason: one abandons an edit and leaves the previous Filter standing, the other abandons it and clears it.
