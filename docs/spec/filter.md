...
- **`unknown:` is the only reason key**, because `enum Unknown` is the only closed reason set in the design (core-api.md:80, versioned by the wire `schema` integer). The table above names two of its three reasons: `SubmoduleUninitialized` was added after this vocabulary was settled and has no keyword yet, so no `unknown:` term reaches it. Naming one is a decision this document owns rather than a gap the code should close on its own, and `unknown_keyword`'s exhaustive match stops a fourth reason arriving unnoticed. `ProbeError`'s variants are unspecified, and a `reason:` key matching rendered error text was refused: it would make the language depend on error wording, and it would decide a type [core-api.md](core-api.md) owns. When those variants are specified, `failed:<variant>` slots into this shape with no change to the grammar.
...
The one named exception: [repo-management.md](repo-management.md)'s "Once accepted" pins a row an in-flight management run or fan-out Action still has its own work pending against, and a pinned row is visible without matching for as long as that pin holds. The header's own match count and the set `a` selects still read the Filter alone, unwidened by any pin, since a pinned row is held past the Filter rather than claimed to satisfy it; only the visible rows and, transitively, an empty-Selection Action's own targets can include it, and only for a run that cannot itself be started again until the one holding the pin is over.

The cost, stated plainly: while a Filter is active a child row whose own Repo did not survive draws the orphan marker in place of the `└` connector ([layout-and-provenance.md](layout-and-provenance.md)), and a Worktree's display name is not parent-qualified, so a filtered Worktree row does not say which Repo it belongs to and the detail pane is the only discriminator. That is the same cost [head.md](head.md) already accepts for a detached row's `branch` cell.
...
The Filter applies **live, on every keystroke**. Enter commits it and returns focus to the list. Esc while typing abandons the edit and restores the previously committed Filter; a second Esc clears the committed Filter, the last rung of [keybindings.md](keybindings.md)'s unwind stack. `list`'s own `Alt+/` clears a committed Filter directly, in one press, without touching the Selection, the detail pane or a running Action; the unwind rung still exists beside it, for a hand already reaching for Esc.

`Alt+/` also fires from inside the line itself, bound there separately since `global` is suspended in `input` and could never carry `list`'s own row across ([keybindings.md](keybindings.md)'s "The contexts"). `Alt+/` clears whatever is narrowing the list: the rule is the same in both contexts, only what's narrowing the list differs. Inside the line that is the live draft, the committed Filter underneath it, or both, so it closes the line the same way Esc does but clears the draft and the committed Filter together, and the line always reopens over nothing rather than whatever was last applied. An empty draft over no committed Filter narrows nothing, so `Alt+/` there is unavailable exactly as it is from the list with no Filter committed: it raises the Notice and leaves the line open. Esc and `Alt+/` stay two distinct gestures for exactly this reason: one abandons an edit and leaves the previous Filter standing, the other abandons it and clears it.
...
The Filter line takes one real row directly above the footer, shifting the list up once when `/` opens it, and the footer switches to its own three hints, `enter apply  esc cancel  alt-/ clear filter` at 43 columns, degrading to `enter apply  esc cancel` on a frame too narrow for the third ([keybindings.md](keybindings.md)). The completion list **overlays** the bottom of the list area, anchored to the Filter line and growing upward, capped at 8 rows and scrolling with `Ctrl+J`/`Ctrl+K` beyond that. It never resizes the list, because the list already narrows on every keystroke and rows that also jump under a resize are unreadable.
