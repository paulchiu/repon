...
**Ignored**:
A Repo or Worktree a `[[repo]]` entry excludes: discovered and probed like any other, never operated on, and undrawn until [the ignored toggle](docs/spec/keybindings.md#the-ignored-toggle) shows it. The `ignore` operation is what sets and clears it, and it is the state the row's own mark names. Not a Set's `exclude` glob, which decides what is discovered at all rather than what is drawn and acted on.
_Avoid_: Excluded, for the row (the word is spoken for: it names the Skip reason a run records for one of these rows)

**Management operation**:
One of the three built-in entries in the Action palette that change what Repon operates on, remove a Repo from the machine, or fast-forward one to its upstream: `ignore`, `delete` and `sync`. Built in rather than configured, so the three names are reserved and a config-defined Action may not take one. Fans out over the Selection and shares the Action confirm gate, and runs no child process at all beyond `sync`'s own optional `before_sync` and `after_sync` hooks. A Management operation's own run moves onto a background thread once the confirm gate is accepted, so it never blocks the draw loop, and Esc cancels it between rows rather than mid-row.
